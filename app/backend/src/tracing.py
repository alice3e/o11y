"""
OpenTelemetry трейсинг для Backend Service
"""
import os
import logging
...
def setup_tracing(app):
    """
    Настройка OpenTelemetry трейсинга для Backend Service
    """
    try:
        ...
        # Автоматическая инструментация Cassandra драйвера
        CassandraInstrumentor().instrument()
        
        logger.info("OpenTelemetry tracing setup completed successfully")
        logger.info("Enabled instrumentations: FastAPI, HTTPX, Cassandra")
