...
def profile_context(profile_name: str):
    ...
    try:
        ...
        filename = f"backend-service_{profile_name}_{timestamp}.prof"
...
def list_available_profiles() -> list:
    """Возвращает список доступных профилей для backend-service"""
    if not os.path.exists(PROFILES_DIR):
        return []
    
    profiles = []
    for filename in os.listdir(PROFILES_DIR):
        if filename.endswith('.prof') and 'backend-service' in filename:
            filepath = os.path.join(PROFILES_DIR, filename)
